...
def simplex_method(c, A, b, B):
    ...
    Параметры:
    c (numpy.ndarray): Вектор коэффициентов целевой функции.
    A (numpy.ndarray): Матрица ограничений.
    b (numpy.ndarray): Вектор правых частей ограничений.
    B (list): Начальные базисные индексы.
    
    Возвращает:
    Оптимальный план x или сообщение о неограниченности функции.
    """
    m, n = A.shape
    x = np.zeros(n)
    AB = A[:, B]
    
    # Преобразуем b в 1D массив, если это необходимо
    b = b.flatten()
    ...
    while True:
        AB = A[:, B]
        cB = c[B]
        u = cB @ np.linalg.inv(AB)  # вектор потенциалов
        ...
        z = np.linalg.solve(AB, A[:, j0])  # направление изменений
        ...
        success, A_inv = is_invertible_and_inverse(np.linalg.inv(AB), A[:, j0], k)
        if not success:
